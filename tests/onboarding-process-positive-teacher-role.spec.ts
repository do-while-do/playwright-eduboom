...
test('Positive scenario - Teacher role selected', async ({ page }) => {
    ...
    await onboarding.selectSubject(onboarding.subjectsContainer[0]); // Matematyka
    ...
    await dashboard.expectSubjectForGradeVisible(7, onboarding.subjectsContainer[0]); // Matematyka
});
